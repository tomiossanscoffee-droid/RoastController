...
# 焙煎機に送れるプロファイル。発熱の検証用は較正の当てはめには使わない
# (測るのが重量ではなく時間なので、いまの当てはめの仕組みに乗らない)。
SENDABLE_PROFILES = {**CALIBRATION_PROFILES, "exotherm": EXOTHERM_PROBE_PROFILE}
SENDABLE_KIND_LABELS = {**CALIBRATION_KIND_LABELS, "exotherm": "発熱の検証"}
