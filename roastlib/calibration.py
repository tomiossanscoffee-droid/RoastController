...
# ■ 焙煎の発熱を測るためのプロファイル(2026-08 追加)
# いまのモデルは、乾物の分解を常に吸熱として扱っている。しかし焙煎士は
# 「2ハゼに入るとガスを絞らないと焙煎が進んでしまう」と言う。実際、2ハゼの
# 時点で豆の熱容量は71J/Kしかなく、観測されるRoR 9℃/分を保つのに要るのは
# 10.6W。文献の発熱(100〜250 kJ/kg)を1〜3分で放出すると23〜175Wで、
# 必要な熱の2〜16倍にあたる。無視できる量ではない。
#
# ところが較正用プロファイルは2つとも最後まで吸入温度を上げ続けるので、
# 発熱と入熱を区別できない。分けて測るには「2ハゼに入ったところで吸入温度を
# 落とし、そのまま保つ」しかない。
#
# ■ 何を測るか
# 焙煎後の重量では測れない。浅煎り側で1g前後の質量モデルの誤差が分かっており、
# 発熱の効果(0.5g程度)がその中に埋もれるため。
# 測るのは<b>「吸入温度を落とし始めてから、2ハゼの音が消えるまでの秒数」</b>。
# 耳と時計だけで測れて、質量モデルの誤差の影響を受けない。
#   発熱なし(いまのモデル) 16秒 / 100 kJ/kg 21秒 / 175 kJ/kg 29秒 / 250 kJ/kg 43秒
# 豆温度が±2℃ずれていても、発熱なしは12〜19秒、175 kJ/kgは25〜34秒で、
# 範囲が重ならない。
#
# ■ 形の決め方
# 650秒(実測の2ハゼ622秒に対して28秒の余裕)まで深煎り用と同じように上げ、
# そこから20秒で218℃まで落として保つ。落とす先は、発熱の有無で秒数がいちばん
# 分かれ、かつモデルの誤差に振られにくい温度として選んだ(205℃だと差が8秒、
# 222℃だと差は19秒に開くが、豆温度の誤差にも振られやすくなる)。
# 保持は90秒。それ以上続けても分かることは増えず、豆が焼けるだけ。
EXOTHERM_PROBE_PROFILE = {
    "name": "豆温度モデル 発熱の検証用",
...
# ■ 昇温を遅くして測るためのプロファイル(2026-09 追加)
# 既存の3本は1ハゼがそれぞれ7.4分・7.5分・7.4分と、ほぼ同じ速さに固まっている。
# 同じ速さの焙煎を何本足しても新しい情報は増えない。実際、いまの実測13点で
# 支えられる定数は4個までで、5個目を足すとAICc(小標本補正つき情報量規準)は
# 悪化する。つまり「証拠が足りない」状態にある。
#
# ■ 何のために焼くか
# いちばん決まっていないのは H_ENDO(乾物が分解するときの吸熱)。当てはめる
# データの選び方で336〜4833と14倍も動く。豆温度そのものを測れないため、
# H_ENDO と U0(熱の入りやすさ)が「入る熱を増やし、同時に吸う熱を増やす」
# 方向に縮退しているのが原因。
# 昇温を遅くすると、熱が入る時間に対して分解が進む時間の比が変わるので、
# この2つが違う応答を示す。感度解析では、この1本を足すだけで決まっていない
# 方向が10.5倍改善し、H_ENDOの誤差は602%→39%になる。
#
# ■ 形の決め方
# 1ハゼが13.1分に来るまで遅らせた(既存は7.4分)。メーカー公表の最長15分の
# 範囲に収めつつ、既存3本からいちばん離れる形を探した結果。
# 定数が±15%外れた27通りすべてで1ハゼが起きることを確認してある
# (1ハゼ 748〜842秒 / 焙煎後 37.4〜42.3g)。
#
# ■ 何を測るか
# 1ハゼの開始時刻と、焙煎後の重量の2つだけ。1ハゼは終了まで届かない設計なので
# (終了時点でも爆ぜ続けている)、1ハゼ終了と2ハゼは測れない。
# 予測は 1ハゼ 13分06秒 / 焙煎後 40.0g。
#
# なお K_WATER(蒸発の速さ)と CRACK_SPREAD(ハゼの幅)は、この形でもほとんど
# 改善しない。ハゼ時刻と最終重量という2種類の観測では原理的に決まりにくく、
# 追加のプロファイルを何本焼いても埋まらない。
SLOW_RAMP_PROFILE = {
    "name": "豆温度モデル 遅い昇温の検証用",
...
# 焙煎機に送れるプロファイル。発熱の検証用と遅い昇温の検証用は較正の当てはめには
# 使わない。前者は測るのが重量ではなく時間で、後者は1ハゼ終了が測れないため、
# どちらもいまの当てはめの仕組み(fcStart/fcEnd/roastedGを揃いで使う)に乗らない。
# 測った値は焙煎記録のメモに残してもらい、モデルを見直すときにまとめて使う。
SENDABLE_PROFILES = {**CALIBRATION_PROFILES,
                     "exotherm": EXOTHERM_PROBE_PROFILE,
                     "slowramp": SLOW_RAMP_PROFILE}
SENDABLE_KIND_LABELS = {**CALIBRATION_KIND_LABELS,
                        "exotherm": "発熱の検証",
                        "slowramp": "遅い昇温"}
